$(function () {
    var setting = {
      ...
      callback: {
       ...
       zTree, rMenu, jsonViewData, curDragNodes, isClick = false, isDblClick = false;
    ...
    function addTreeNode() {
      ...
      var newNode = { id: 'view' + curId, name: "视点" + (curId), dropInner:false};
    ...
    function removeTreeNode() {
      ...
      var nodes = zTree.getSelectedNodes(),
          nodeId = nodes[0].id,
          pid = GetQueryString("proj") || 'wptest';
      /*console.log(pid);
      console.log(nodeId);*/
      ...
      if (isClick) {
        setTimeout(function () {
          if (isClick) {
            ...
            console.log('节点单击事件'+ event+treeId+ treeNode+ clickFlag);
    ...
    $.ajax({
      url: 'http://123.57.69.5/api/Project/GetAllViewpoint',
      type: 'get', //数据发送方式
    ...
    // 添加视点API
    function addData (proId, jsonData) {
     ...
     /* DataStr = encodeURI(Data);
      console.log(DataStr)*/

      var proId = proId || 'wptest';

      $.ajax({
        url: 'http://123.57.69.5/api/Project/AddViewpoint?ProjectID=' + proId + '&ViewpointData=' + DataStr,
        type: 'get', //数据发送方式
        contentType:"application/x-www-form-urlencoded; charset=utf-8",
        dataType: 'json', //接受数据格式 (这里有很多,常用的有html,xml,js,json)
        /*data: {
          'ProjectID': proId || 'wptest',
          'ViewpointData': Data
        }, //要传递的数据*/
        error: function() { //失败
